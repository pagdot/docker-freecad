# [pagdot/freecad](https://github.com/pagdot/freecad)
...
Simply pulling `pagdot/freecad:latest` should retrieve the correct image for your arch, but you can also pull specific arch images via tags.
...
* Image version number:
    ...
    ```bash
    docker inspect -f '{{ index .Config.Labels "build_version" }}' pagdot/freecad:latest
    ```
...
* Update the image:
    ...
    ```bash
    docker pull pagdot/freecad:latest
    ```
...
```bash
git clone https://github.com/pagdot/docker-freecad.git
cd docker-freecad
docker build \
  --no-cache \
  --pull \
  -t pagdot/freecad:latest .
```
